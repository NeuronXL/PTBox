...
- 导入的私钥与原有内嵌公钥匹配，使用当前账户 DPAPI 加密保存；私钥及备份均位于 Git 忽略目录，未放入发行包。
- 干净打包并通过 ReleaseTool 签名及完整性验证：安装包 105229721 字节，SHA256 `e1a8c9f5116fe65763542d8179cf1462dc9749f371d973eefaed7ecf4ae30293`。
- 同一干净构建目录生成独立便携 ZIP，默认应用列表为空，带有显式便携标记。
- 本次不在开发机执行覆盖安装。实际 1.1.1 → 1.1.2 安装、重启及个人配置保留由测试用户验收，步骤见 `docs/releases/1.1.2.md`。
